...
const Scheduletime = () => (
  ...
  <StaticQuery
    query={graphql`
      {
        wordpressWpSchedule(sort: {fields: id, order: ASC}) {
            edges {
            node {
                title
                    acf {
    ...
    <div className="container">
        <div className="row">
            <div className="col-lg-12">
                <div className="schedule-table-tab">
                    <ul className="nav nav-tabs" role="tablist">
                        {props.wordpressWpSchedule.edges.map(ttttt => (
                            <li className="nav-item" key={ttttt.id}>
                            {/* <li className="nav-item" key={ttttt.node.categories.slug}> */}
                                <Link to={`/${ttttt.node.categories.slug}`} className="nav-link" activeclassName='active' data-toggle="tab" role="tab">
                                    {ttttt.name}
                                </Link>
                            </li>
                        ))}
          ...
          {props.wordpressWpSchedule.edges.map(items => (
                        ...
                        <div className="tab-pane active" id="tabs-1" role="tabpanel">
                            <div className="schedule-table-content">
                                <table>
                                    ...
                                    <tbody>
                                    {props.wordpressWpSchedule.edges.map(ddd => (
                                        <tr>
                                            <td className="event-time">{ddd.schedule_time}</td>
                                            <td className="break hover-bg"><h5>{ddd.schedule_time}</h5></td>
                                        </tr>
                                    ))}
                                        {/* <tr>
